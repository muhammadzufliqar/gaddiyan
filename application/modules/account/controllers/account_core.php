...
class Account_core extends CI_Controller {
...

	{
		...
		load_template($data,$this->active_theme,'login');
...

    {
...

		{
			...
			if($package->price<=0) {
				...
				$this->session->set_flashdata('msg', '<div class="alert alert-danger">You cannot update to a free package</div>');
...

	{
        ...
        load_template($data,$this->active_theme,'register');
 ...
 /*   	if($this->session->userdata('package_id')=='')
        ...
        $data['content'] 	= load_view('register',$value,TRUE);*/
...

	{
		...
		/*$enable_pricing = get_settings('autocon_settings','enable_pricing','Yes');
...
*/
		...
		/*	if($enable_pricing=='Yes')
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-success">Your account has been created successfully and send you confirmation link. Please check your email.</div>');
...

	{
		...
		load_template($data,$this->active_theme,'confirmation_view');
	...
	{		
...

		{
		    ...
		    $order = $this->autocon_model->get_feature_payment_data_by_unique_id($uniqid);
...

		    {
...

		    	{
...

		    		{
						...
						$this->autocon_model->update_post_by_id(array('featured'=>1),$post_id);
	...
	public function send_notification_mail($data)
...

	{
		...
		$this->email->from('mmtpaypal@bookit.com', 'Registration');
...

	{
		...
		$userdata['user_email'] = 'scmondal@webhelios.com';
...

	{
       ...
       $data['home_contact']      =  $this->show_model->get_contact(1);
        ...
        load_template($data,$this->active_theme,'recover_view');
...

	{
...

		{
			...
			$this->session->set_flashdata('msg', '<div class="alert alert-success">A email is send to your inbox/spam.Check that email.</div>');
...

	{
...

		{
			...
			$this->form_validation->set_message('useremail_match_check', 'Email is not in our database');
