...

	{
...

		{
			...
			if(constant("ENVIRONMENT")=='demo') {
				...
				$state_id 				= $this->autocon_model->get_location_id_by_name($this->input->post('state'),'state',$this->input->post('country'));

				$city_id 				= $this->autocon_model->get_location_id_by_name($this->input->post('city'),'city',$state_id);
...
#contact
	...
	public function contact()
	{
		if($this->uri->segment(4) == 'edit'){
		  ...
		  $value['contact']   = $this->admin_model->get_all_contact($this->uri->segment(5));
			
		}else if($this->uri->segment(4) == 'delete'){
			$this->admin_model->delete($this->uri->segment(5));
		}else{
			
        $this->form_validation->set_rules('name',	'Name', 		'required|xss_clean');
        $this->form_validation->set_rules('ph',	'Phone No', 		'required|xss_clean');
		...
		{       
	            ...
	            $userdata['ph'] = $this->input->post('ph');
	            $userdata['email'] 	= $this->input->post('email');
	            $userdata['zip_code'] 	= $this->input->post('zip_code');
	            $userdata['address'] 	= $this->input->post('address');
				$this->admin_model->add_contact($userdata,$id);
				$this->session->set_flashdata('msg', '<div class="alert alert-success">Data Saved</div>');
		}
		}
		$value['title']	= 'Contact Us';
		$value['contacts']   = $this->admin_model->get_all_contact();
		$data['content'] 	= $this->load->view('admin/contact/contact_view',$value,TRUE);
		$this->load->view('admin/template/template_view',$data);
	}
}
