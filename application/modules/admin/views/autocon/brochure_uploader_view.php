...
<div id="trainingrecords-attachments" target=".trainingrecords-attachment-list">
    ...
    <form action="<?php echo site_url('admin/autocon/uploadbrochure');?>" method="post" enctype="multipart/form-data">
        <ol class="filelist">
        </ol>
        <input type="file" name="brochure" style="height:auto;" >
        <input type="submit" class="btn btn-info" value="Upload File" style="margin-top:10px;">
    </form>
...
//                alert(xhr.responseText);
                ...
                if(response.error==0)
                {
                    ...
                    window.parent.jQuery('#car_brochure_input').attr("value",response.name);
                    window.parent.jQuery('#car_brochure_input').trigger('change');
