...

{
...

	{
...

		{
			...
			$CI->load->view('themes/'.$theme.'/header',$data);

			$CI->load->view('themes/'.$theme.'/'.$view,$data);

		    $CI->load->view('themes/'.$theme.'/footer',$data);
...

{
	...
	function load_template($data=array(),$theme='',$tmpl='index')
...
if ( ! function_exists('get_related_vehicle'))
...

{
	...
	function get_related_vehicle ($brand_id,$id)
...

	{
		...
		$CI->db->from('posts AS a');

		$CI->db->where('a.id !=',$id);

		$CI->db->where('a.brand',$brand_id);
